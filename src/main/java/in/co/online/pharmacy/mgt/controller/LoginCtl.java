...
import in.co.online.pharmacy.mgt.model.RoleModel;
...
public class LoginCtl extends BaseCtl {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (id > 0) {
			...
			ServletUtility.forward(OPSView.LOGIN_VIEW, request, response);
			return;
		}
		if (session.getAttribute("user") != null) {
			ServletUtility.redirect(OPSView.WELCOME_CTL, request, response);
			return;
		}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (OP_SIGN_IN.equalsIgnoreCase(op)) {
			...
			ServletUtility.redirect(OPSView.USER_REGISTRATION_CTL, request, response);
		return;
		}
